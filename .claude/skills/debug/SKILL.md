...
The system extracts only authentication variables (`CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`) from `.env` and mounts them for sourcing inside the container. Other env vars are not exposed for security.
...
**Docker mount syntax:**
```bash
# Readonly: use :ro suffix
-v /path:/container/path:ro

# Read-write: no suffix
