...
Messages must start with the trigger pattern (default: `@Nanomi`):
- `@Nanomi what's the weather?` → ✅ Triggers Claude
- `@nanomi help me` → ✅ Triggers (case insensitive)
- `Hey @Nanomi` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Nanomi what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Nanomi what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Nanomi add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Nanomi remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Nanomi list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Nanomi remember I prefer dark mode` | Add to global memory |
...
```
User: @Nanomi remind me every Monday at 9am to review the weekly metrics
...
```
User: @Nanomi at 5pm today, send me a summary of today's emails
...
From any group:
- `@Nanomi list my scheduled tasks` - View tasks for this group
- `@Nanomi pause task [id]` - Pause a task
- `@Nanomi resume task [id]` - Resume a paused task
- `@Nanomi cancel task [id]` - Delete a task

From main channel:
- `@Nanomi list all tasks` - View tasks from all groups
- `@Nanomi schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Nanomi add group "Name"` in main |
